...
alert('Order Deleted Succesfully')
    ...
    public function UpdateStatus($riderId, $amtToRecieve, $status)
    {
        ...
        if ($this->Run()) {
            ...
            if ($this->Run()) {
                $this->Query('SELECT pId from `products`,`orderditem` WHERE products.pId = orderditem.proId AND orderditem.orderId = :orderId');

                $this->Bind(':orderId', $this->orderId);
                $pId = $this->Single();

                $this->Query('UPDATE `products` SET `quantity`= quantity-1 WHERE `pId` = :proId');
                $this->Bind(':proId', $pId['pId']);
                if ($this->Run()) {
                    echo "<script type='text/javascript'>
