...
class Products extends Database
{
    ...
    public function checkCompareProduct($pName)
    {

        // $this->Query("SELECT * FROM products WHERE pName = :pName AND id =:Id Limit 1");
        $this->Query("SELECT * FROM products_categories,products,categories,users WHERE products_categories.p_id = products.pId AND categories.cat_id = products_categories.cat_id AND products.pName = :pName AND users.userId = products.sellerId  Limit 2");
        $this->Bind(':pName', $pName);
        return $this->All();
    }
    ...
    public function addProduct($catId, $quantity)
    {

        $this->Query('SELECT * FROM `products` WHERE products.pName = :pName AND products.sellerId = :sellerId');
        ...
        if ($this->Total()) {
            ...
            $date = date("Y/m/d");
            $this->Query('INSERT INTO `products` (`pName`, `pDesc`, `pPrice`, `pImage`, `inStock`,`sellerId`,`quantity`,`created_at`,`updated_at`) VALUES (:proName,:proDesc,:proPrice,:proImage,:stock,:sellerId,:quantity,:created_at,:updated_at )');
            $this->Bind(':proName', $this->pName);
            $this->Bind(':proDesc', $this->pDesc);
            $this->Bind(':proPrice', $this->pPrice);
            $this->Bind(':proImage', $this->pImage);
